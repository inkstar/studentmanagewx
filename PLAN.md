...
- 验收标准：导出图包含紫蓝渐变头部（含 1/1 与状态胶囊）、2 列信息卡、三段内容白卡与灰底页脚，整体观感接近模板。
- 风险与回滚：风险为 canvas 字体与 Web 字体差异导致细节不完全一致；回滚方式为 `git revert` 本阶段提交并恢复上一版导出样式。

## [UTC+8 2026-03-04 01:59] Phase 30 - 课程操作图标极简化（Done）
- 目标：将课程记录操作图标从表情风格替换为更简洁的线性符号风格。
- 改动文件：`pages/lesson/lesson.wxml`、`pages/lesson/lesson.wxss`、`PLAN.md`。
- 验收标准：操作按钮图标统一为单色极简符号（编辑/复制/查看/导图/删除），视觉不再呈现 emoji 风格。
- 风险与回滚：风险为个别字体下符号字形细节存在差异；回滚方式为 `git revert` 本阶段提交并恢复上一版图标集合。

## [UTC+8 2026-03-04 01:57] Phase 29 - 课程导出图片移动端适配重构（Done）
- 目标：针对手机端阅读体验重做课程导出图片 UI，解决原版式拥挤和观感粗糙问题。
- 改动文件：`pages/lesson/lesson.js`、`pages/lesson/lesson.wxml`、`pages/lesson/lesson.wxss`、`PLAN.md`。
- 验收标准：导出图具备更清晰层级（标题区/信息卡/三段内容卡/底部信息），字体与留白适配移动端，长文本可读性明显改善。
- 风险与回滚：风险为部分机型 canvas 字体渲染差异导致个别行宽波动；回滚方式为 `git revert` 本阶段提交并恢复上一版导出样式。
